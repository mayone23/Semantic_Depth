...
path = "/home/uesr/Desktop/Cityscapes/"
# our CLI parser
...
img_data = CityscapesDataset(path, split='train', mode='fine', augment=True)
img_batch = torch.utils.data.DataLoader(img_data, batch_size=4, shuffle=True)
print(img_data)
...
for epoch in range(epochs):
    ...
    for idx_batch, (imagergb, labelmask, labelrgb, depth_label) in enumerate(img_batch):
       ...
       # plt.imshow()
        ...
        loss1 = recon_loss_func(y, y_)

        file_loss.write(str(loss1.item())+'for semantic'+"\n")
        loss1.backward()
        gen_optimizer.step()
        ...
        if idx_batch % 4 == 0:
            ...
            #v_utils.save_image(depth_label, "./result/depth_image_{}_{}.png".format(epoch, idx_batch))
