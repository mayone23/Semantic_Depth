...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, root, split='train', mode='fine', augment=False):
        ...
        self.targets_depth = []
        print(os.path.join(self.root,'leftImg8bit',split))
        self.mapping = {
            0: 0,  # unlabeled
            1: 0,  # ego vehicle
            2: 0,  # rect border
        ...
        for city in os.listdir(self.images_dir):
            ...
            for file_name in os.listdir(img_dir):
                ...
                target_name_dep = '{}_{}'.format(file_name.split('_leftImg8bit')[0], '{}_color.png'.format(self.mode))
                # target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0], '{}_color.png'.format(self.mode))
                self.targets_depth.append(os.path.join(target_dir, target_name_dep))
                target_name_sem = '{}_{}'.format(file_name.split('_leftImg8bit')[0], '{}_labelIds.png'.format(self.mode))
                # target_name = '{}_{}'.format(file_name.split('_leftImg8bit')[0], '{}_color.png'.format(self.mode))
                self.targets_semantic.append(os.path.join(target_dir, target_name_sem))
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            sem_target = TF.resize(sem_target, size=(128+10, 256+10), interpolation=Image.NEAREST)
            dep_target = TF.resize(dep_target, size=(128+10, 256+10), interpolation=Image.NEAREST)
            ...
            sem_target = TF.resize(sem_target, size=(128, 256), interpolation=Image.NEAREST)
            dep_target = TF.resize(dep_target, size=(128, 256), interpolation=Image.BILINEAR)
